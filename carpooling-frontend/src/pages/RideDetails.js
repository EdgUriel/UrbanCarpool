  ...
  useEffect(() => {
    ...
    window[`initMap${mapId}`] = () => {
      const map = new window.google.maps.Map(
        document.getElementById(`map${mapId}`),
        {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
      ...
      {ride && (
        <div className="space-y-2 text-gray-700 dark:text-gray-300">
          ...
          <p>
            <strong>Price:</strong> {ride.seatPrice} MXN
          </p>
          <p>
            <strong>Car:</strong> {ride.car} ({ride.plate})
          </p>
          <p>
            <strong>Travel Preference:</strong> {ride.travelPreference}
          </p>
        </div>
      )}
...
const RideDetails = ({ ride }) => {
  ...
  const [passengerId, setPassengerId] = useState(null); // Obtener el ID del pasajero de alguna manera
      ...
      const response = await axios.post(
        ...
        null,
        {
          params: {
            ...
            segmentStartId: ride.segmentStartId, // Obtener estos valores de alguna manera
            segmentEndId: ride.segmentEndId,
          },
        }
      );
    ...
    <div className="container mx-auto p-4">
      <div className="text-center mb-12">
        ...
        <p className="text-xl text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
          ...
          Experience convenience, comfort, and affordability with our trusted
          drivers.
        </p>
      </div>
      ...
      {selectedRide && (
        <ConfirmDialog ride={selectedRide} onClose={handleCloseDialog} />
      )}
      <div>
        <h2>
          {ride.origin} to {ride.destination}
